...
{	
	...
	var Template=function(config)
	{
		...
		{	
			...
			if(parseInt($this.config.template.mainPreloader)===1)
			{
		...
		this.setRequestCurrent=function()
		{
			...
			if(String(hash.substring(0,$this.config.template.hashBang.length))===String($this.config.template.hashBang))
		...
		{
			return(String($this.requestCurrent)===String(request));
		};
		...
		{
			return(parseInt($this.config.template.requestType)===1 ? true : false);
		};
		...
		{
			return(parseInt($this.config.template.requestType)===2 ? true : false);
		};
		...
		{	
			...
			if($this.isHashRequest())
			{
				...
				{
					if(parseInt($this.config.template.menuOpenStart)===1)
		...
		this.doRequest=function()
		{
			...
			else if(parseInt($this.requestCurrent.length)!==0)
			{
				$this.checkRequest();
		...
		this.getPage=function(data)
		{
			...
			if(parseInt($this.config.template.requestType)===2)
			{
		...
		{
			$.getScript('script/page.js',function() 
			{
		...
		this.setToggleOption=function(option,data)
		{
			...
			try
			{
				if(typeof(data.animate)!==typeof(undefined))
		...
		this.createMainPreloader=function(data)
		{
			...
			{			
				...
				{
					var count=parseInt(index)===parseInt(countImage-1) ? countPeriodLast : countPeriod;
		...
		this.removeMainPreloader=function(data)
		{
			...
			if(parseInt(preloader.length)===1)
			{
		...
		this.animateTimeoutDuration=function()
		{
			...
			if(parseInt(page.length)!==1) return;
		...
		this.selectMenuItem=function(object)
		{
			...
			if(parseInt(menuResponsive.length)===1)
			{
		...
		{		
			...
			function() 
			{
				if(String(action)==='open') $this.setClass('template-vertical-navigation-bar-open');
		...
		this.getRandomTag=function(selector,parent)
		{
			...
			if(parseInt(element.length)===0) return(false);
		...
		this.doEvent=function(event)
		{
			...
			{
				if(typeof(event.complete)!==typeof(undefined)) event.complete.apply();
			};                  
		};
		...
		{
			if(($this.isHashRequest()) || (String(request)==='menu/open') || (String(request)==='menu/close'))
			{
		...
		this.getMeta=function(name)
		{
			...
			{
				if(String($this.config.page[i].hash)===String($this.requestCurrent))
				{
					try
					{
			...
			if(($this.isEmpty(value)) || (typeof(value)===typeof(undefined)))
			{
				try
				{
